...
- Klaviyo applies suppression jobs in the background: in `klaviyo_sandbox` they took two to four hours, and the job status stayed `processing` and reported every profile as skipped even after they were suppressed. So the import submits the jobs and doesn't wait for them.
...
- 2026-09-26: Fourth review fixes. Every command checks the key belongs to the instance's expected account; `klaviyo dedupe check` verifies imports read-only; suppression counts shown as "submitted"; malformed `suppressions` JSON is a row error; retried writes warn; export timestamps keep fractions of a second; exports interrupted after their last page don't refetch; only identifier and control columns are trimmed on read. Recovery from an interrupted import stays "re-run the file" (documented in `docs/DEDUPE_IMPORT.md`).
