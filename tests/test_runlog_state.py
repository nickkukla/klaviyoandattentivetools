...
def test_jobs_are_saved_and_updated(tmp_path):
    ...
    store.add_job("klaviyo_us", {"id": "j1", "status": "PENDING"})
    store.update_job("klaviyo_us", "j1", status="COMPLETE")
    assert store.jobs("klaviyo_us") == [{"id": "j1", "status": "COMPLETE"}]
    with pytest.raises(ValueError):
        store.add_job("klaviyo_us", {"id": "j1"})
    with pytest.raises(KeyError):
        store.update_job("klaviyo_us", "nope", status="x")
